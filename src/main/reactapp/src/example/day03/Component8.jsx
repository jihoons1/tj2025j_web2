...
export default function Component8( props ){
    // 리엑트 훅(갈고리) : 1.useState(상태) 2.useEffect(효과)
    // [1] useEffect : 특정한( 현 컴포넌트=실행/종료 ) 시점에 *정의함수*가 실행
    // [2] 사용법 : 
    //      2-1 : import { useEffect } from "react"
    //      2-1 : mount/update  : useEffect( 정의함수 ) 또는 useEffect( ()=>{} )
    ...
    // 3-1 : 1. 최초실행 2. 재실행
    useEffect( ()=> { console.log('(1)컴포넌트(함수) 탄생 , 업데이트 ' ); } )

    const [ count , setCount ] = useState( 0 );

    // 3-2 : 1. 최초실행 2.[특정상태변경]재실행
    useEffect( ()=> {
        console.log('(2) 컴포넌트(함수) 탄생,  *특정*업데이트 ');
