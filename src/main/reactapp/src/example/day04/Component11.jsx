...
export default function Component11(props){
    ...
    const inputRef = useRef( null );    // 1. omport { useRef } from "react" 또는 자동완성
                                        // 2. useRef( 초기값 )
    //
    const 등록함수 = () => { 
        console.log(inputRef);              // 3. inputRef : 현재 참조중인 객체 정보 { current : 참조값 }
        console.log(inputRef.current);      // 4. useRef.current : 참조값
        inputRef.current.focus();           // - 포커스( 마우스커서 )
        console.log( inputRef.current.value ); // 단순 입력 이라면 useState 보다 useRef
        // useRef - web 화면에 입력받은값 노출시키지 못함 재렌더링 불가
    }
    ...
    return(<>
            ...
            <form ref={formRef}>
                <input name="textData" id="textDat" class="textData"/>
