...
export default function Component4( props ){
    ...
    return( <>
            {/* JSX 구역 */ }
            <h3> 이름: {obj.name} 나이: {obj.age} </h3>
            <SubComp key1 = "value1" key2 = "value2"></SubComp>
            <SubComp name = "강호동" age = "41"></SubComp>
...
function SubComp( props){
    ...
    return (<>
        <h4> SubComp </h4>
        <SubSubComp key3="value3"></SubSubComp>
    </>)
} //func end

let count2 = 0; // 전역변수 특징 : 서로 다른 함수들간의 공유 , 함수 밖에 있는 변수 
...
function SubSubComp( props){
    ...
    return(<>
        ...
        <h6> 지역변수 count : {count} / 전역번수 count2 : {count2} </h6>
     </>)
}