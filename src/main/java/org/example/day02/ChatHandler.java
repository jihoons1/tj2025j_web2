...
public class ChatHandler extends TextWebSocketHandler {
    ...
    // ** 클라이언트 소켓들이 서버에 접속된 클라이언트 소켓 명단 목록 *****
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        System.out.println("[클라이언트 웹소켓]" +  message);
        // [*] 서버가 클라이언트에게 메세지 보내기 new TextMessage("문자열")
//        session.sendMessage(new TextMessage("메세지 잘 받았습니다.") );
        // 메세지를받을세션.sendMessage( )
        // 서버로 부터 메세지를 보내온 클라이언트 소켓(session) 에게 메세지는 보내는 예제

        // 1. 특정한 클라이언트 소켓으로 부터 받은 메세지를 **현재 접속된** 다른 클라이언트소켓들에게 메세지를 보내기
        // 나 -- > 안녕하세요 --> 카카오 서버 --> 상대
        // 상대 --> 안녕하세요 --> 카카오 서버 --> 나
        for (WebSocketSession clientSocket : 접속명단){
            // 접속명단(리스트)내 저장(접속)된 클라이언트소켓들을 하나씩 커낸다.
            // 클라이언트소켓 하나씩 . sendMessage 함수를 이용한 서버가 받은 메세지를 보내준다.
            clientSocket.sendMessage(message); // message : 서버가 받은 메세지
        }
