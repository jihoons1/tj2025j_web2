...
public class CrawlingService {
    ...
    public Map<String , String> task1() {
        ...
        // 1-6 : 셀레니움(웹드라이버) 잠시 대기 , new WebDriverWait( 셀레니움객체 , Duration.ofSeconds( 초 )  );
            // * 왜? 대기하나요? 동적페이지는 JS(fetch)가 정보를 가져올때까지 정보가 없으므로 가다린다.
        ...
        // 1-7 : 대기후 크롤링할 HTML CSS 식별하기 분석하기 , 권장 : 식별자가 1개가 아닌 상위 식별자를 넣어서 중복방지
            ...
            // (1) 지역   .info_location .tit_location         , 자손선택자 : 식별자 1 식별자2 , 자식선택자 : 식별자선택
        WebElement location
        ...
        // 1-8 : 크롤링한 요소(HTML마크업의 텍스트를 추출하여 map/dto저장
    ...
    public List<String> task2(){
        ...
        WebDriver webDriver = new ChromeDriver(chromeOptions);
        // 2-4 : 크롤링할 웹주소
        String URL = "https://cgv.co.kr/cnm/cgvChart/movieChart/89867";
        ...
        for (int i = 1; i<=5 ; i++) {
            ...
            for (WebElement element : webElements) { // 여러개 리뷰 요소들을 하나씩 조회
                ...
                if (list.contains(text) ) { // [중복방지] ** 만약에 스크롤 내리고 리시트내 앞전의 리뷰
                    continue; // 가장 가까운 반복문 으로 이동 -> 2-7
                }
                list.add(text);
            }
