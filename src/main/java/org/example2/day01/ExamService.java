...
public class ExamService {
    ...
    public ExamEntity post( ExamEntity examEntity ){
        // 1. 저장할 엔티티를 (매개변수)받는다.
        // 2. .save() , 리포지토리의 저장 메소드
        ExamEntity saveEntity =
                examRepository.save( examEntity ); // insert 자동처리
        // 엔티티객체(레코드=행=dto 1개) 엔티티(테이블)
        return saveEntity;
    }
    // 2. R , 전체조회
    ...
    public ExamEntity put(ExamEntity examEntity){
        ...
        // 2. save( 수정할엔티티 )
            // 만약에 지정한엔티티에 pk가 없으면 *생성
            // 만약에 지정한엔티티가 pk가 존재하면 *수정
    ...
    @Transactional // 트랜잭션이란? 여러개 sql에서 하나라도 실패하면 모두 실패
    public ExamEntity put2( ExamEntity examEntity) {
        // 1. 수정할 엔티티 조회한다. , findAll() , findById( pk 조회 )
