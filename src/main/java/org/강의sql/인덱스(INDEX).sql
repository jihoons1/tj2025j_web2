...
create index idex_name on employee( name ); # 확인  3 -> 2
# [4] 쿼리 성능 조회 : select 문 앞에
explain analyze
select * from employee where name = '유재석';
# [5] 인덱스 삭제
# drop index 인덱스명 on 테이블명;
drop index idex_name on employee;	# 확인 5 -> 2 -> 4 , 3 -> 4
# [6] 복합 인덱스 생성
...
explain analyze select * from employee where dept_id = 1;	-- 첫번쨰 인덱스 가능
...
# JOIN : PK - FK 아니더라도 조인 가능하다. UNIQUE
# FK 에 인덱스 추가
create index index_dep on employee( dept_id );
# FK 에 인덱스 추가 후 JOIN 하면 조인 속도가 증가한다 . *
select * from employee e inner join department d on e.dept_id = d.dept_id;

# [8] 문자열 검색 : 자연어 NLP( 사람이 사용하는 언어/ 번역속도 줄이기 )  VS 기계어(컴퓨터 2진수)
# type이 text, char , varchar 가능하다.
