...
public class JwtService {
    ...
    private String secret = "22222222222222222222222222222222";
    // [2] 개발자가 정의한 비밀키 이용한 sha-256 알고리즘 적용
    ...
    public Claims getClaims( String token ){
            return Jwts.parser()
                    ...
                    .getBody(); // 검증에 성공한 토큰의 (저아된데이터)클레임들 반환
    }
    ...
    // 특정한 데이터를 직접적으로 보여주지 않고 징표를 대신 보여주는 구현체
    // 웹에서 데이터 숨기기( 보안 )

    // JWT 토큰 가능하지만 , 데이터들 간의 서로다른 토큰 과 복잡한 계산식 추가
    // SHA-256 알고리즘 사용하여 비밀키( 임의, 개발자만들기 , 토큰 풀기 위한 비밀번호)를 32글자이상
    // 1-2 : 내가 만든 임의의 토큰에 사용되는 계산식의 비밀번호
    private final String 비밀번호 = "11111111111111111111111111111111"; // 난수 키보다는 직접생성한 키 권장
    ...
    public String 토큰생성( String 데이터 ){
        // 1-1 토큰 생성 : 라이브러리
        String token = Jwts.builder() // .builder() : 빌더패턴( 생성자 대신에 함수형 객체생성) 이용한 토큰
                .claim("key" , 데이터 ) // .claim( key , value ) : 토큰에 넣을 데이터 대입
                .setIssuedAt( new Date() ) // .setIssuedAt( 토큰발급날짜/시간 )
                // .setExpiration( new Date( System.currentTimeMillis()+1000 * 초 ) ) // .setExpiration( 토큰만료시간 )
                // .setExpiration( new Date( System.currentTimeMillis()+1000 * 60 * 60 * 24 * 7 ) ) // 60초 // 60분 // 24시간 // 7일
                .setExpiration( new Date( System.currentTimeMillis()+1000 * 30 ) ) // 30초
                .signWith( 비밀키 , SignatureAlgorithm.HS256 ) // 알고리즘 이용한 토큰에 서명하기 + HS256알고리즘
