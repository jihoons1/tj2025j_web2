...
public class JwtAuthFiler extends OncePerRequestFilter {
    // 내가만든 토큰 인증 방법을 (시큐리티방식)usernamepasswordauthenticationToken 혼합
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if ( request.getCookies() != null ) { // 쿠키가 존재하면
            for (Cookie cookie : request.getCookies()) { // 모든 쿠키들을 반복문 돌려서
                if (cookie.getName().equals("loginUser")) { // 로그인재 쿠키가 존재하면
        ...
        if (token != null && jwtService.checkToken(token) ){ // 토큰이 유효 하면
            ...
            // new UsernamePasswordAuthenticationToken( uid , null , List.of( new SimpleGrantedAuthority("ROLE_권한명1") , new SimpleGrantedAuthority("ROLE_권한명") ) );
            // 2-2 : 권한 생성
